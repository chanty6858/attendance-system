// Pure-JS crypto helpers so the scanner works in non-secure contexts
// (e.g. http://<lan-ip>:8080 on a phone), where crypto.subtle / randomUUID
// are unavailable. Produces the same HMAC-SHA256 hex as node:crypto.
...
export function hmacSha256Hex(secret, text) {
  ...
  return toHex(hmacSha256(enc.encode(secret), enc.encode(text)));
}
